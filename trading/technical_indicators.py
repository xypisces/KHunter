...
.. deprecated::
    本模块为旧接口的兼容层，所有指标计算已迁移到 `indicators/` 模块。
    新代码请直接使用 `from indicators import ma, rsi, kdj, ...` 或 `indicators.CachedIndicators`。
"""
import pandas as pd
import numpy as np
from typing import Dict, Tuple, Optional

from indicators import ma as _ma, rsi as _rsi, kdj as _kdj, macd as _macd
from indicators import atr as _atr, bollinger as _bollinger
...
class TechnicalIndicators:
    ...
    .. deprecated:: 使用 `indicators.CachedIndicators` 替代
    """
    ...
    def calculate_ma(self, df: pd.DataFrame, period: int, stock_code: str = "") -> pd.Series:
        ...
        if key not in self.cache[stock_code]:
            self.cache[stock_code][key] = _ma(df['close'], period)
    ...
    def calculate_atr(self, df: pd.DataFrame, period: int, stock_code: str = "") -> pd.Series:
        ...
        if key not in self.cache[stock_code]:
            self.cache[stock_code][key] = _atr(df, period)
    ...
    def calculate_rsi(self, df: pd.DataFrame, period: int, stock_code: str = "") -> pd.Series:
        ...
        if key not in self.cache[stock_code]:
            self.cache[stock_code][key] = _rsi(df, period)
    ...
    def calculate_bollinger_bands(self, df: pd.DataFrame, period: int, multiplier: float, stock_code: str = "") -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        if key_mid not in self.cache[stock_code] or key_upper not in self.cache[stock_code] or key_lower not in self.cache[stock_code]:
            mid, upper, lower = _bollinger(df, period, multiplier)
    ...
    def calculate_macd(self, df: pd.DataFrame, short_period: int, long_period: int, signal_period: int, stock_code: str = "") -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        if key_macd not in self.cache[stock_code] or key_signal not in self.cache[stock_code] or key_hist not in self.cache[stock_code]:
            dif, dea, hist = _macd(df, short_period, long_period, signal_period)
            self.cache[stock_code][key_macd] = dif
            self.cache[stock_code][key_signal] = dea
            self.cache[stock_code][key_hist] = hist
        else:
            dif = self.cache[stock_code][key_macd]
            dea = self.cache[stock_code][key_signal]
